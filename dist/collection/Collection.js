...
class Collection {
   ...
   */
  defaultSort() {
    const sortedEntries = Array.from(this.entries()).sort((a, b) =>
  ...
  combineEntries(collection) {
    const combinedEntries = [...this.entries(), ...collection.entries()];
  ...
  difference(collection) {
    const diffEntries = Array.from(this.entries()).filter(
  ...
  intersect(collection) {
    const intersectEntries = Array.from(this.entries()).filter(([key]) =>
      collection.has(key),
    );

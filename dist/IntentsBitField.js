...
class IntentsBitField {
  ...
  add(...ints) {
    const filter = ints.filter(i => {
      if (typeof i !== "number") {
        throw new errorcollection_1.BitFieldError(
          `specified "${i}" is not correct`,
        );
      }
    });
    for (const i of ints) {
  ...
  toArray() {
    ...
    for (const [flag, bit] of Object.entries(IntentBits_1.IntentBits)) {
      if (this.has(bit)) {
        const intent = TelegramIntentBits_1.TelegramIntentBits[flag].replace(
          /_/g,
          "_",
        );
        arr.push(intent);
...
function decodeIntents(intentsBitField) {
  ...
  for (const [flag, bit] of Object.entries(IntentBits_1.IntentBits)) {
    if (
      intentsBitField === null || intentsBitField === void 0
        ? void 0
        : intentsBitField.has(bit)
    ) {
      const intent = TelegramIntentBits_1.TelegramIntentBits[flag].replace(
        /_/g,
        "_",

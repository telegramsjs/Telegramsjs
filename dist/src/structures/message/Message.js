...
class Message extends Base {
    ...
    _patch(data) {
        ...
        if ("caption_entities" in data) {
             ...
             */
            this.captionEntities = new MessageEntities(data.caption, data.caption_entities);
        }
        if ("entities" in data) {
            /**
        ...
        if ("new_chat_members" in data) {
             ...
             * @type {User[] | undefined}
             */
            this.newChatMembers = data.new_chat_members.map((user) => new User(this.client, user));
        }
        ...
        if ("new_chat_photo" in data) {
             ...
             */
            this.newChatPhoto = data.new_chat_photo.map((photo) => new Photo(this.client, data.new_chat_photo));
        }
        ...
        if ("proximity_alert_triggered" in data) {
            ...
            const proximityAlertTriggered = {
                traveler: new User(this.client, data.proximity_alert_triggered.traveler),
    ...
    reply(text, options = {}) {
        if (!this.chat) {
            throw new TelegramError("Could not find the chat where this message came from in the cache!");
        }
